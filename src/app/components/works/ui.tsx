...
import type { FilterId, MVWork, Photo, AudioTrack, SortMode } from './types';
import { categoryPalette } from './palette';
import type { TilePalette } from './palette';
import { ImageModal } from '../ImageModal';

const FALLBACK_TILE: TilePalette = {
...
function BentoTile({
    ...
    return (
        ...
        >
            ...
            <div aria-hidden className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500" style={{ background: `radial-gradient(120% 80% at 50% 120%, ${p.accent}66, transparent 60%)` }} />
            <div className="relative z-10 flex flex-col justify-between h-full p-4 md:p-5">
                <div className="flex items-start justify-between gap-2">
                    <span className="w-9 h-9 rounded-xl flex items-center justify-center backdrop-blur-md transition-colors duration-300 group-hover:bg-white/90" style={{ background: 'rgba(0,0,0,0.28)', border: '1px solid rgba(255,255,255,0.3)' }}>
                        <Icon className="w-4 h-4 text-white transition-colors duration-300 group-hover:text-[#0b1020]" />
                    </span>
                    <span className="film-title leading-none text-2xl md:text-4xl text-white/85" style={{ textShadow: '0 2px 12px rgba(0,0,0,0.5)' }}>{c}</span>
                </div>
                <div>
                    <div className="metadata text-[9px] md:text-[10px] text-white/70 uppercase tracking-[0.18em] mb-1">Explore</div>
                    <div className="text-white font-semibold tracking-tight leading-tight text-base md:text-2xl drop-shadow-[0_2px_10px_rgba(0,0,0,0.7)]">{f.label}</div>
                </div>
            </div>
        </motion.button>
    );
...
export function BentoSelector({
    ...
    return (
        <>
            <style>{`
        @keyframes bento-pulse{0%,100%{opacity:.5}50%{opacity:1}}
        @keyframes bento-drift{0%{background-position:0 0}100%{background-position:44px 44px}}
      `}</style>
...
/* ===== AUDIO: ROW (compact) + PLAYER (expanded) + LIST (engine) ===== */
function AudioRow({ track, active, playing, onRowClick, onPlayClick }: { track: AudioTrack; active: boolean; playing: boolean; onRowClick: () => void; onPlayClick: () => void }) {
    return (
...
function AudioPlayer({ track, playing, prog, wave, onToggle, onPrev, onNext, shuffle, onShuffle, repeat, onRepeat, onSeek, onClose }: {
...
/* ===== CREDIT CARD ===== */
export function CreditCard({ work }: { work: MVWork }) {
    return (
