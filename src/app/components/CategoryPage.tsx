...
export function CategoryPage() {
    ...
    const { films, credits, photos, audio } = filterId
        ? getList(filterId)
        : { films: [], credits: [], photos: [], audio: [] };
    ...
    const SectionHead = ({ title }: { title: string }) => (
        <div className="mb-6 flex items-center gap-4">
            <h3 className="film-title text-2xl md:text-3xl text-white shrink-0">{title}</h3>
    ...
    return (
        <section className="relative min-h-screen">
            ...
            <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}
                ...
                style={{ background: 'linear-gradient(180deg, rgba(5,7,16,0.95), rgba(5,7,16,0.8))', borderBottom: `1px solid ${pal.accent}44` }}>
                        ...
                        {showVideo && (
                            <section>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                                    {sortedFilms.map((p, i) => (
                                        <div key={`film-${p.category}-${p.id}`} className={p.category === 'Short Film' ? 'md:col-span-2' : ''}>
                                            <FilmProject project={p} index={i} isInView={true} />
                                        </div>
                                    ))}
                                    {sortedCredits.map((w) => (
                                        <div key={`mv-${w.category}-${w.id}`}>
                                            <CreditCard work={w} />
                                        </div>
                                    ))}
                                </div>
                                    ...
                                    <div key={g.name} className="mb-12 last:mb-0">
                                        {!isAll && photoGroups.length > 1 && (
                                            <div className="mb-5 flex items-center gap-4">
                                                <h4 className="film-title text-lg md:text-xl text-white/90 shrink-0">{g.name}</h4>
                                                <span className="metadata text-white/40 text-[10px] tabular-nums">{g.items.length}</span>
