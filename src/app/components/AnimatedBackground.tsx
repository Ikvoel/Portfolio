...
const STREAK_LINES = `repeating-linear-gradient(90deg, transparent 0, transparent 22px, rgba(199,199,242,0.05) 22px, rgba(199,199,242,0.05) 24px)`;
const STREAK_SHAFTS = `linear-gradient(92deg, transparent 0%, rgba(255,255,255,0.10) 12%, transparent 22%, rgba(199,199,242,0.08) 34%, transparent 44%, rgba(255,236,210,${0.10 * (WARM || 0.5)}) 58%, transparent 68%, rgba(255,255,255,0.09) 80%, transparent 92%)`;
...
.ab-streak{position:absolute;inset:-20%;mix-blend-mode:screen;pointer-events:none}
.ab-streak-lines{position:absolute;inset:0;filter:blur(1.5px);opacity:.55;background-size:200% 100%;animation:ab-streak 26s linear infinite}
.ab-streak-shafts{position:absolute;inset:0;filter:blur(10px);opacity:.6;background-size:220% 100%;animation:ab-shafts 34s linear infinite}
...
.ab-fa,.ab-fb,.ab-fc,.ab-flow,.ab-l1,.ab-l2{will-change:transform,opacity}
.ab-streak-lines,.ab-streak-shafts{will-change:background-position}
...
@keyframes ab-streak{0%{background-position:0% 0}100%{background-position:200% 0}}
@keyframes ab-shafts{0%{background-position:0% 0}100%{background-position:-220% 0}}
...
@media (max-width:768px){
  .ab-fb,.ab-flow,.ab-fc,.ab-lw2,.ab-grain,.ab-streak-shafts{display:none}
  .ab-fa{filter:blur(22px)} .ab-l1{filter:blur(26px)} .ab-streak-lines{opacity:.35}
}
@media (prefers-reduced-motion:reduce){.ab-fa,.ab-fb,.ab-fc,.ab-flow,.ab-l1,.ab-l2,.ab-streak-lines,.ab-streak-shafts{animation:none!important}}
`;
...
export function AnimatedBackground({ hue = 0 }: { hue?: number } = {}) {
  return (
    <div className="fixed inset-0 z-0 overflow-hidden pointer-events-none" style={hue ? { filter: `hue-rotate(${hue}deg)` } : undefined}>
      ...
      <div className="ab-streak">
        <div className="ab-streak-lines" style={{ background: STREAK_LINES }} />
        <div className="ab-streak-shafts" style={{ background: STREAK_SHAFTS }} />
      </div>
